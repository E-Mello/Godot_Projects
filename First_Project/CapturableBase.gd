...
export (Color) var enemy_color = Color(0.090196, 0.070588, 0.407843)
...
func check_whether_base_can_be_captured():
	...
	if majority_team == Team.TeamName.NEUTRAL:
		...
		# stop the capture countdown, if it is started
		team_to_capture = Team.TeamName.NEUTRAL
		capture_timer.stop()
	else:
		print("Newe team has majority in base, starting capture clock")
